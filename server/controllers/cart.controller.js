...
export const clearCartController = async (request, response) => {
    try {
        ...
        // Clear user's shopping cart array
        const updateResult = await UserModel.findByIdAndUpdate(
            userId,
            { $set: { shopping_cart: [] } },
            { new: true }
        );
        console.log('User shopping cart updated:', updateResult);

        // Verify cart is cleared
        const remainingCartItems = await CartProductModel.find({ userId });
        if (remainingCartItems.length > 0) {
            // If items still exist, try one more time
